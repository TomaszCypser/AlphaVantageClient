...
namespace AlphaVantageClient.Stock.Mappers
{
    public class MonthlyResponseMapperProfile : Profile
    {
        ...
        {
            CreateMap<StockTimeSeriesApiResponse?, Models.MonthlyResponse>()
                ...
                .ConvertUsing((source, dest, context) =>
                {
                    ...
                    (
                        context.Mapper.Map<Serialization.MetaData, Models.MonthlyMetaData>(source.MetaData),
                        context.Mapper.Map<Dictionary<string, Serialization.TimeSeries>, Dictionary<string, Models.MonthlyTimeSeries>>(source.TimeSeries)
                    );
                });

            CreateMap<Serialization.MetaData, Models.MonthlyMetaData>()
                .IgnoreAllPropertiesWithAnInaccessibleSetter()
                .ConvertUsing((source, dest, context) =>
                {
                    if(source is null) throw new ArgumentNullException(nameof(source));
                    ...
                    return new Models.MonthlyMetaData
                    (
                        source.Information,
                        source.Symbol,
            ...
            CreateMap<Dictionary<string, Serialization.TimeSeries>, Dictionary<string, Models.MonthlyTimeSeries>>()
                .IgnoreAllPropertiesWithAnInaccessibleSetter()
                .ConvertUsing((source, dest, context) =>
                {
                    if(source is null) throw new ArgumentNullException(nameof(source));
                    var mappedResponse =  new Dictionary<string, Models.MonthlyTimeSeries>();
                    foreach(var item in source)
                    {
                        mappedResponse.Add(
                            item.Key,
                            context.Mapper.Map<Serialization.TimeSeries, Models.MonthlyTimeSeries>(item.Value)
                            );
                    }
                    return mappedResponse;
                });

            CreateMap<Serialization.TimeSeries, Models.MonthlyTimeSeries>()
                .IgnoreAllPropertiesWithAnInaccessibleSetter()
                .ConvertUsing((source, dest, context) =>
                {
                    if(source is null) throw new ArgumentNullException(nameof(source));
                    ...
                    return new Models.MonthlyTimeSeries
                    (
                        openConverted,
                        highConverted,
                        lowConverted,
